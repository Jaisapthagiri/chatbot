...
const ChatBox = () => {
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-32 max-md:mt-14 2xl:pr-40'>
      ...
      <form onSubmit={onSubmit} className="bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#88069F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-1 items-center">
        <select onChange={(e) => setMode(e.target.value)} value={mode} className='text-sm pl-3 pr-2 outline-none'>
          <option className='dark:bg-purple-900' value="text">Text</option>
          <option className='dark:bg-purple-900' value="image">Image</option>
        </select>
        <input value={prompt} onChange={(e) => setPrompt(e.target.value)} type="text" className='flex-1 w-full text-sm outline-none' placeholder='Ask Anything ...' required />
